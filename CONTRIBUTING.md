...
- **Renovate**: runs from `.github/workflows/renovate.yml` on a GitHub runner — daily at 08:00 Asia/Jakarta, plus **Run workflow** when you want it sooner — and authenticates with the `RENOVATE_TOKEN` secret, without which the Renovate step fails. It lives here rather than on the hosted app because a 3GB job cannot resolve this workspace: `pnpm install --lockfile-only` peaks near 1.6GB and the kernel killed the job. `renovate.json` decides what may open: every non-major update batches into one pull request inside the Monday window, majors and pre-1.0 minors wait until you tick them on the **Dependency Dashboard** issue, GitHub Actions majors arrive together, and the overrides in `pnpm-workspace.yaml` are left alone. A quiet weekday is the schedule working rather than a broken install. Its pull requests run the same gates as any other, so a bump onto a package with a new advisory fails `audit`, and one that drags in a vulnerable transitive fails `review`.
