...
**Amendment: the tenant CSP is now per-tenant, along the axis a form needs.** The consequence above accepted per-template tightening of the two `'unsafe-inline'` directives as a future direction. The first move in that direction turned out to be the opposite operation — a *widening*, for `connect-src` and `form-action` — but it establishes the mechanism the tightening will use.

Contact submissions go to Web3Forms (`api.web3forms.com`) rather than to an endpoint Plinth owns, and that is the decision the CSP has to accommodate. Submissions are the one class of tenant visitor data the platform would otherwise store, secure and delete on request; delivering them straight to the tenant's inbox keeps them out of Plinth entirely, and out of its backups, its logs and its subject-access obligations.

**What is stored per tenant is a delta, not a policy.** `TenantHostMapping` gains `formOrigins?: string[]`, and the worker composes its CSP from the base list plus whatever that tenant is allowed. Storing the finished policy per tenant was the obvious alternative and is worse in a way that only shows up later: the security posture becomes data rather than code, every subsequent tightening needs a re-sync of every tenant before it takes effect, and a stale KV entry keeps serving last month's policy with nothing to indicate it. With a delta, the policy stays reviewable in one file and a stale entry can only ever be wrong about a form.

The allowance is derived at sync time from the **published** snapshot, not the draft. A key removed in the draft but still live would otherwise have its form blocked before the change shipped; one added in the draft would be granted before it existed. Every promote re-derives it, so removing a form closes the widening rather than leaving it open until somebody notices. The worker additionally filters what it reads against a fixed allowlist — KV carries no schema of its own, and without that filter a bad write could widen a tenant to any origin at all.

`connect-src` and `form-action` move together, deliberately. The first covers the enhanced `fetch`, the second the native POST the page falls back to without JavaScript. Granting one and not the other breaks exactly half the form — whichever half nobody tested.

**The access key is not a secret, and this is where that is written down.** It authorises posting to one form and nothing else, and it ships in the page's HTML by design, which is Web3Forms' own model. It lives in `content_drafts.document`, so it is also in the weekly database dump and in every retained version snapshot. That is acceptable for a value the published page already hands to every visitor — but it is named here so nobody has to work it out during an audit.

**Two things the bundle budget and the test pool made explicit.** The enhancement script reads its endpoint from the form's own `action` attribute rather than importing the shared constant: importing it cost 22.5 KB, because the schema barrel that defines it pulls zod into the browser, and the budget was the only thing that would have caught that. Reading the attribute is also stronger than the import was — the enhanced path and the no-JavaScript path cannot post to different places when they are the same string. The worker keeps its own copy of the origin for the same reason it has no runtime dependencies at all, held honest by a test, which may import what the bundle may not.
